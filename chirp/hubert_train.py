...
def filtered_hubert_loss_from_outputs(outputs: hubert.ModelOutputs,
  ...
  targets = getattr(outputs, "targets")
...
def hubert_loss_from_outputs(outputs: hubert.ModelOutputs, alpha: float,
  ...
  mask_idc = getattr(outputs, "mask_idc")
...
def hubert_and_supervised_loss_from_outputs(outputs: hubert.ModelOutputs,
                                            alpha: float, label: jnp.ndarray,
                                            genus: jnp.ndarray,
                                            family: jnp.ndarray,
                                            order: jnp.ndarray,
  ...
  return hubert_loss + readout_loss_mult * readout_loss
...
def quantizer_loss(outputs: hubert.ModelOutputs, quant_loss_mult: float,
  ...
  quant_loss = getattr(outputs, "quantization_loss")
...
def taxonomy_cross_entropy(outputs: hubert.ModelOutputs, label: jnp.ndarray,
  ...
  if isinstance(outputs.label, list):
    assert not taxonomy_loss_weight
    means = []
    for l in outputs.label:
      means.append(
          jnp.mean(optax.sigmoid_binary_cross_entropy(l, label), axis=-1))
    mean = jnp.sum(jnp.stack(means, axis=0), axis=0)
  else:
    mean = jnp.mean(
...
def make_metrics_collection(prefix: str, alpha: float, quant_loss_mult: float,
  ...
  metrics_dict = {
      ...
      "hubert_and_supervised_loss":
          clu_metrics.Average.from_fun(
              functools.partial(
                  hubert_and_supervised_loss_from_outputs,
                  alpha=alpha,
                  readout_loss_mult=readout_loss_mult)),
  }
...
class TrainState:
  ...
  quant_opt_state: optax.OptState
  hubert_opt_state: optax.OptState
...
class ModelBundle:
  model: nn.Module
  hubert_optimizer: optax.GradientTransformation
  quant_optimizer: optax.GradientTransformation
  key: jnp.ndarray
...
def initialize_model(
    dataset_info: tfds.core.DatasetInfo, model_config: config_dict.ConfigDict,
    rng_seed: int, input_size: int, learning_rate: float, workdir: str,
  ...
  print("num_classes dict {}".format(num_classes))

  # Initialize the quantizer.
  base_quantizer = quantizers.VectorQuantizerEnt(
  ...
  peak = learning_rate / 1e-10
  hubert_learning_rate = optax.piecewise_interpolate_schedule(
      "linear",
      init_value=1e-10,
      boundaries_and_scales={
          int(num_train_steps / 2): peak,
          num_train_steps: 1e-10
      })

  # Initialize optimizer and handle constraints
  std_to_fwhm = jnp.sqrt(2 * jnp.log(2)) / jnp.pi
  hubert_optimizer = optax.chain(
      optax.adam(learning_rate=hubert_learning_rate),
      optax.masked(
  ...
  hubert_opt_state = hubert_optimizer.init(params)

  quant_optimizer = optax.chain(
      ...
      optax.masked(
          project(4 * std_to_fwhm, model.frontend.kernel_size * std_to_fwhm),
          mask_by_name("gabor_std", params)))
  quant_opt_state = quant_optimizer.init(params)
  ...
  train_state = TrainState(
      step=0,
      params=params,
      quant_opt_state=quant_opt_state,
      hubert_opt_state=hubert_opt_state,
  ...
  return ModelBundle(model, hubert_optimizer, quant_optimizer, key,
                     ckpt), train_state
...
def train(model_bundle,
  ...
  def get_update_step(only_quantizer=False, freeze_quantizer=False):
    ...
    def update_step(key, batch, train_state, mask_key):
      ...
      quant_step, hubert_step = None, None
      quant_opt_state, hubert_opt_state = None, None
      if not only_quantizer:
        hubert_step = get_step("train___hubert_and_supervised_loss")
      if not freeze_quantizer:
        quant_step = get_step("train___quantizer_loss")

      # model_state has only the batch_norm stats which only appear in the
      # late feature extractor (conformer).
      if quant_step:
        (_, (train_metrics, model_state)), quant_grads = jax.value_and_grad(
            quant_step, has_aux=True)(train_state.params,
                                      train_state.model_state)
        quant_grads = jax.lax.pmean(quant_grads, axis_name="batch")
        quant_updates, quant_opt_state = model_bundle.quant_optimizer.update(
            quant_grads, train_state.quant_opt_state, train_state.params)
      if hubert_step:
        (_, (train_metrics, model_state)), hubert_grads = jax.value_and_grad(
            hubert_step, has_aux=True)(train_state.params,
                                       train_state.model_state)
        hubert_grads = jax.lax.pmean(hubert_grads, axis_name="batch")
        hubert_updates, hubert_opt_state = model_bundle.hubert_optimizer.update(
            hubert_grads, train_state.hubert_opt_state, train_state.params)
      ...
      for k, v in train_state.params.items():
        if "quantizer" in k:
          if freeze_quantizer:
            ...
            print("Updating {} with quantizer loss".format(k))
            updates[k] = quant_updates[k]
        else:
          if only_quantizer:
            print("freezing {}".format(k))
            updates[k] = v
          else:
            print("Updating {} with hubert loss".format(k))
            updates[k] = hubert_updates[k]
  ...
  if reload_quantizer:
    # Only train HuBERT.
    hubert_and_readout_update_step = get_update_step(freeze_quantizer=True)
  else:
    # Train both jointly, or first quantizer only and then both.
    if num_quantizer_pretrain_steps:
      quantizer_update_step = get_update_step(only_quantizer=True)
      hubert_and_readout_update_step = get_update_step(freeze_quantizer=True)
    else:
      joint_update_step = get_update_step(only_quantizer=False)
  ...
  for step in range(initial_step, num_train_steps + 1):
    with jax.profiler.StepTraceAnnotation("train", step_num=step):
      ...
      if step < num_quantizer_pretrain_steps:
        ...
        train_metrics, train_state = quantizer_update_step(
            step_key, batch, train_state, mask_key)
      elif reload_quantizer or num_quantizer_pretrain_steps > 0:
        # Train only the HuBERT and readout. This happens if the quantizer was
        # frozen from the start, or if its pretraining has completed.
        train_metrics, train_state = hubert_and_readout_update_step(
            step_key, batch, train_state, mask_key)
      else:
        # Joint training.
        train_metrics, train_state = joint_update_step(step_key, batch,
