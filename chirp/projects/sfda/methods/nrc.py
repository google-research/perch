...
class NRCLoss(clu_metrics.Metric):
    ...
    def from_model_output(
        ...
        **_,
        ...
        Returns:
            An instance of NRCLoss.
        """

        nn_loss = - (nn_weight *
                     (probabilities[:, None, :] * nn_probability).sum(axis=-1)).sum(axis=-1)  # [batch_size]

        extended_nn_loss = - (extended_nn_weight * (
            probabilities[:, None, None, :] * extended_nn_probability).sum(axis=-1)).sum(axis=1)  # [batch_size]
    ...
    def merge(self, other: "NRCLoss") -> "NRCLoss":
        return type(self)(
            ...
            extended_nn_loss_sum=self.extended_nn_loss_sum + other.extended_nn_loss_sum,
    ...
    def compute(self):
        ...
        marginal_entropy = losses.label_ent(
            probabilities=probabilities_marginal)
        return (
        ...
        **_,
        ...
        nn_loss = -(
            ...
            * (
                ...
                * (dot_product(probabilities[:, None, :], nn_probability)).sum(axis=1)
            )
        ...
        extended_nn_indices = jnp.stack(
            [
                self.compute_nearest_neighbors(
                    batch_feature=method_state["dataset_feature"][sample_nn_indices],
                    dataset_feature=method_state["dataset_feature"],
    ...
    def get_adaptation_metrics(
        ...
        metrics_dict = vars(
            adapt.get_common_metrics(
                supervised=supervised, multi_label=multi_label)
        )["__annotations__"]
