...
class SQLiteUsearchDB(interface.GraphSearchDBInterface):
  ...
  Attributes:
    ...
    ui: The USearch index. This is only populated when a new embedding is added
      to the database.
    ui_disk_view: A disk view of the USearch index.
  ...
  def commit(self):
    self.db.commit()
    if self.ui.size > self.ui_disk_view.size:
  ...
  def insert_embedding(
    ...
    if self.ui.size == 0 and self.ui_disk_view.size > 0:
      # We need to load the disk view into memory.
      self.ui.load(self._usearch_filepath.as_posix())
  ...
  def count_embeddings(self) -> int:
    ...
    return max(self.ui.size, self.ui_disk_view.size)
  ...
  def get_embedding(self, embedding_id: int) -> np.ndarray:
    if self.ui.size > 0:
      ...
      emb = self.ui_disk_view.get(embedding_id)
    if emb is None:
  ...
  def get_embeddings(
    ...
    if self.ui.size > 0:
      ...
      embs = self.ui_disk_view.get(embedding_ids)
    if None in embs:
      raise ValueError(f'Embeddings {embedding_ids} not found.')
...
def _setup_sqlite_tables(cursor):
  """Create the needed tables int he SQLite databse."""
  cursor.execute("""
