...
class ConstantEarlyFeatureExtractor(nn.Module):
  ...
  @nn.compact
  def __call__(self, inputs: jnp.ndarray, *unused_args,
...
class HuBERTTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    (self.model_bundle, self.train_state,
     self.learn_rate_schedule) = self.initialized_model
  ...
  def _get_test_config(self,
                       config_module=hubert_base_pq) -> config_dict.ConfigDict:
    ...
    config.init_config.model_config.late_feature_extractor = ConstantLateFeatureExtractor(
    )
    ...
    def step(params, model_state):
      ...
      hubert_loss = jnp.mean(
          hubert_train.hubert_loss_from_outputs(
              model_outputs, alpha=self.alpha))
      return hubert_loss
  ...
  def test_train_one_step(self):
    ...
    hubert_train.train(
        *self.initialized_model,
        ...
        num_quantizer_pretrain_steps=self.config.train_config
