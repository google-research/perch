...
@pytest.fixture(name='ebird_taxonomy_csv_str')
def fixture_ebird_taxonomy_csv_str():
  return ('TAXON_ORDER,CATEGORY,SPECIES_CODE,PRIMARY_COM_NAME,SCI_NAME,ORDER1,'
...
@pytest.fixture(name='taxonomy_info')
def fixture_taxonomy_info():
  return pd.DataFrame({
...
@pytest.fixture(name='broken_down_taxonomy_info')
def fixture_broken_down_taxonomy_info(taxonomy_info):
...
def test_wikidata_species_codes(mock_sparql_wrapper, taxonomy_info):
  # Simulate a code collision to test the overrides kwarg.
  name_to_code = dict(
      zip(taxonomy_info['Scientific name'].map(lambda n: n.replace(' ', '-')),
  ...
  mock_sparql_wrapper.return_value.queryAndConvert.return_value = dict(
      results=dict(bindings=[
          dict(
              eBird_taxon_ID=dict(value=c), Xeno_canto_species_ID=dict(value=n))
  ...
  function_call = functools.partial(
      ...
      taxonomy_info=taxonomy_info.drop(columns='species_code'),
      overrides=overrides)
  ...
  with mock.patch.object(xeno_canto, '_WIKIDATA_CHECKSUM', new=checksum):
    assert function_call().to_list() == taxonomy_info['species_code'].to_list()
...
def test_load_ebird_taxonomy(mock_run, ebird_taxonomy_csv_str):
  mock_run.return_value = mock.MagicMock()

  # The function should return the correct taxonomy.
  mock_run.return_value.stdout = ebird_taxonomy_csv_str.encode('utf-8')
  taxonomy = pd.read_csv(io.StringIO(ebird_taxonomy_csv_str))
  checksum = hashlib.sha256(taxonomy.to_json().encode('utf-8')).hexdigest()
  with mock.patch.object(xeno_canto, '_EBIRD_TAXONOMY_CHECKSUM', new=checksum):
    assert xeno_canto._load_ebird_taxonomy().equals(taxonomy)

  # Without patching with the correct checksum, the function should raise a
  # RuntimeError.
  with pytest.raises(RuntimeError):
    xeno_canto._load_ebird_taxonomy()

  # A malformed taxonomy (non-unique species codes, for instance) should cause
  # the function to raise an error.
  malformed_csv_str = ebird_taxonomy_csv_str.replace('ostric3', 'ostric2')
...
@pytest.mark.parametrize(
    'ebird_taxonomy_str,taxonomy_info_str,expected_taxonomy_info_str',
    [
...
def test_clean_up_species_codes(ebird_taxonomy_str, taxonomy_info_str,
  ...
  assert to_taxonomy_info(expected_taxonomy_info_str).equals(
      xeno_canto._clean_up_species_codes(
          to_taxonomy_info(taxonomy_info_str),
          to_ebird_taxonomy(ebird_taxonomy_str)))


def test_ensure_species_code_uniqueness(taxonomy_info):
  # If there are no collisions, the taxonomy DataFrame should be left untouched.
  assert xeno_canto._ensure_species_codes_uniqueness(taxonomy_info,
                                                     {}).equals(taxonomy_info)

  # If there is a disallowed collision, the function should raise an exception.
  non_unique_taxonomy_info = taxonomy_info.copy()
  ...
  with pytest.raises(ValueError):
    xeno_canto._ensure_species_codes_uniqueness(non_unique_taxonomy_info, {})

  # If there is an allowed collision, the function merge the colliding rows into
  # the first colliding row and drop the others.
  ...
  assert xeno_canto._ensure_species_codes_uniqueness(non_unique_taxonomy_info, {
      frozenset(non_unique_taxonomy_info.iloc[:2]['Scientific name']):
          second_code
  }).equals(expected_taxonomy_info)


def test_break_down_taxonomy(taxonomy_info, ebird_taxonomy_csv_str,
                             broken_down_taxonomy_info):
  ebird_taxonomy = pd.read_csv(io.StringIO(ebird_taxonomy_csv_str))
  assert broken_down_taxonomy_info.equals(
      xeno_canto._break_down_taxonomy(taxonomy_info, ebird_taxonomy))


@mock.patch.object(xeno_canto.requests, 'Session', autospec=True)
def test_scrape_xeno_canto_recording_metadata(mock_session_cls,
                                              broken_down_taxonomy_info):
  # Mock requests.Session.get to simulate a Xeno-Canto API call.
  mock_session_cls.return_value.get.side_effect = (
      _make_mock_requests_side_effect(broken_down_taxonomy_info))
  ...
  species_code_to_recording_metadata = (
      xeno_canto._scrape_xeno_canto_recording_metadata(
          broken_down_taxonomy_info, progress_bar=False))
  for i, row in broken_down_taxonomy_info.iterrows():
    assert species_code_to_recording_metadata[row['species_code']] == [
        xeno_canto.RecordingInfo(
            ...
            background_species=[[
                'Struthio molybdophanes', 'Rhea americana', 'Struthio camelus'
            ][i]])
    ]
...
def test_create_taxonomy_info(mock_read_html, mock_run, mock_sparql_wrapper,
                              ebird_taxonomy_csv_str, taxonomy_info,
                              broken_down_taxonomy_info):
  name_to_code = dict(
      zip(
          broken_down_taxonomy_info['xeno_canto_query'].map(
              lambda n: n.replace(' ', '-')),
          broken_down_taxonomy_info['species_code']))
  matches = sorted((c, n) for n, c in name_to_code.items())
  mock_sparql_wrapper.return_value.queryAndConvert.return_value = dict(
      results=dict(bindings=[
          dict(
  ...
  taxonomy_info['Status'] = None
  mock_read_html.return_value = [
      taxonomy_info.drop(
          columns=['species_code', 'is_insect', 'no_species_code', 'to_verify'])
  ]

  expected_columns = ('species_code', 'xeno_canto_query', 'scientific_name',
                      'species', 'genus', 'family', 'order', 'common_name')
  expected = broken_down_taxonomy_info.copy().drop(columns=[
      c for c in broken_down_taxonomy_info.columns if c not in expected_columns
  ])
  ...
  with mock.patch.object(
    ...
    with mock.patch.object(
      ...
      assert xeno_canto.create_taxonomy_info(
          xeno_canto.SpeciesMappingConfig()).equals(expected)


@mock.patch.object(xeno_canto.requests, 'Session', autospec=True)
def test_retrieve_recording_metadata(mock_session_cls,
                                     broken_down_taxonomy_info):
  # Mock requests.Session.get to simulate a Xeno-Canto API call.
  mock_session_cls.return_value.get.side_effect = (
      _make_mock_requests_side_effect(broken_down_taxonomy_info))

  expected = broken_down_taxonomy_info.copy()
  ...
  expected['xeno_canto_bg_species_codes'] = [[['ostric3']], [['grerhe1']],
                                             [['ostric2']]]
  assert xeno_canto.retrieve_recording_metadata(
      broken_down_taxonomy_info, progress_bar=False).equals(expected)
